...
def generate_front_setup_files():
    # Fetch all entries from Front Setup Item
    front_setup_items = frappe.get_all('Front Setup Paths', fields=['app_name', 'app_path', 'title', 'extension', 'template'])
    ...
    for item in front_setup_items:
        ...
        #     frappe.msgprint(_(f"File {full_file_path} already exists. Skipping creation."))
        #     continue
...
def get_app_paths(doctype, folder_name=None):
    ...
    if not folder_name:
        folder_name = "blocks" if doctype == "Web Template" else "sections"
...
def generate_page_component_template(docname, doctype, folder_name=None):
    ...
    try:
        ...
        template_content = doc.get('template')
        ...
        frappe.msgprint(f"Component {filename} created successfully at {full_file_path}.")
...
def generate_frontend_component(docname, doctype, folder_name=None):
    ...
    frappe.msgprint(f"Page component {filename} has been created at {full_path}.")
